...
Ext.define('Ext.ux.slidenavigation.View', {
    ...
    onItemTap: function (list, index, target, item, e, eOpts) {
    	...
    	if (!Ext.isFunction(item.raw.handler)) {
    		me._cache[index] = undefined;
    		me.items.getAt(1).removeAll();
    	}
    	list.select(index);
        list.fireEvent('select', list, item, eOpts);
    },
    ...
    onSelect: function(list, item, eOpts) {
		...
		if (me._cache[index] == undefined) {            
            ...
            if (Ext.isFunction(item.raw.handler)) {
                ...
                me._cache[index] = container.add(Ext.merge({}, me.config.defaults, me.slideNavigatorItemArray[index]));
        ...
        if (!Ext.ModelManager.get(model)) {
            Ext.define(model, {
                ...
                config: {
                    idProperty: 'index',
                    fields: [
                        'index', 'title', 'group', 'selected', 'titlename',
                        {
                            name: 'order',
                            defaultValue: 1
                        },{
